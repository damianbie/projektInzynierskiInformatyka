function mainLoop(timer, ev, app)
    imgSize = app.data.inputSize(1:2); timeStart = tic;
    frame = (snapshot(app.cam)); frame = imresize(frame, imgSize);
...
function [app, frame, bboxes, scores, labels] = getBoxPos(app, frame)
    timeStart = tic;    app.wykryteTextArea.Value = ["Brak"];
    [bboxes, scores, labels] = detect(app.data.detector, frame); % detekcja
    if(app.data.isColor == 0)
        frame = cat(3, frame, frame, frame);
    end
    if isempty(labels) == 0 % wykryto jakies obiekty
        headers = [];
...
function frame = sledzenie(timer, ev, app, frame) %algorytm sledzacy
